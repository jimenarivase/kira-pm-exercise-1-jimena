...
Missing fee_disbursement rows for B and C (invisible — no row = no error flag in DB)
wire_international undercharge (looks like a valid fee, just wrong amount)
Client A tier-blend error (recorded fee exists, just slightly wrong)
Client A 2bp subsidiary discount clause — never applied, term sheet clause that has
no DB representation at all
Client C wire pricing — charged with no contractual basis
